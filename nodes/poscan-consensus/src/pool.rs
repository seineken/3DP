...
use poscan_grid2d::{hash_meets_difficulty, Compute, DoubleHash};
use runtime::AccountId;
use sp_core::{H256, U256};
use sp_std::collections::{btree_map::BTreeMap, btree_set::BTreeSet, vec_deque::VecDeque};
use sp_std::sync::Arc;

use sp_consensus_poscan::{CheckMemberError, Difficulty};
...
impl Clone for MiningPool {
	fn clone(&self) -> Self {
		...
		Self {
            ...
            dynamic_difficulty: self.dynamic_difficulty.clone(),
		}
	}
}
...
impl MiningPool {
	...
	fn pre_verify(
		...
		let comp = Compute {
			difficulty: self.curr_meta.as_ref().unwrap().difficulty,
			pre_hash,
			poscan_hash: dh,
		};

		if hash_meets_difficulty(
			&comp.get_work(),
			self.curr_meta.as_ref().unwrap().difficulty,
		) {
			Ok(true)
		} else {
			let comp = Compute {
	...
	pub(crate) fn dynamic_difficulty(
		&self,
		pool_id: AccountId,
		member_id: AccountId,
	) -> U256 {
